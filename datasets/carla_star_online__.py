...
from utils.metrics import get_local_vertices


class StarOnlineDataset(Dataset):
    def __init__(
        self, args, split, num_frames, current_frame, num_vehicles, start_frame=0
    ):
        print(
    ...
    def load_imgs_poses(self, args):
        ...
        bboxes_path = f"{args.datadir}/bboxes.npy"
        ...
        if self.split == "test":
            ...
            if args.has_depth_data:
                depth_imgs = depth_imgs[:, : self.eval_last_frame, ...]
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            # "predicted_poses": self.predicted_poses if self.split == "test" else None,
    ...
    def get_gt_vehicle_poses(self, args):
        ...
        for i in range(self.num_vehicles):
            ...
            pose_files = sorted(
                glob(f"{args.datadir}/poses/{vehicle_dirs[i]}/*.npy"), key=natural_keys
            )
            ...
            poses_ue4[i] = np.stack(poses_ue4[i], axis=0)
        ...
        poses_ue4 = np.stack(poses_ue4, axis=0)
        assert poses_ue4.shape == (
            self.num_vehicles,
            self.num_frames,
            4,
            4,
    ...
    def load_gt_relative_poses(self, args):
        ...
        for j in range(self.num_vehicles):
            pose_files = sorted(
                glob(f"{args.datadir}/poses/{vehicle_dirs[j]}/*.npy"), key=natural_keys
            )
    ...
    def get_noisy_gt_relative_poses(self):
        ...
        for i in range(self.num_vehicles):
            ...
            rot_noise = (
                np.random.randn(self.gt_relative_poses.shape[1] - 1) * np.pi / 16
                - np.pi / 32
            )
            trans_noise = (
                np.random.randn(self.gt_relative_poses.shape[1] - 1, 3) / 100.0
            )
            ...
            noisy_pose_matrix = np.eye(4, dtype=np.float32)[None, ...].repeat(
                self.gt_relative_poses.shape[1], axis=0
            )
            noisy_pose_matrix[:, :3, :3] = Rotation.from_euler(
                "xyz", noisy_rot
            ).as_matrix()
        ...
        assert noisy_poses.shape == (
            self.num_vehicles,
            self.num_frames,
            7,
        ), "Noisy poses are not created correctly!"
