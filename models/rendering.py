...
def sample_pts(rays_o, rays_d, near, far, N_samples, perturb=0, lindisp=False, is_train=True):
...
def render_star_appinit(star_network, pts, viewdirs, z_vals, rays_o, rays_d, N_importance):
    # Pass through coarse network
    rgb_map0, disp_map0, acc_map0, weights, depth_map0 = star_network(pts, viewdirs, z_vals, rays_d, 
                                                                      is_coarse=True)
    
    # Hierarchical volume sampling
    z_vals_mid = .5 * (z_vals[...,1:] + z_vals[...,:-1])
    z_samples = sample_pdf(z_vals_mid, weights[...,1:-1], N_importance, det=(not star_network.training))
    z_samples = z_samples.detach()
    z_vals, _ = torch.sort(torch.cat([z_vals, z_samples], -1), -1)

    pts = rays_o[...,None,:] + rays_d[...,None,:] * z_vals[...,:,None] # [N_rays, N_samples + N_importance, 3]

    rgb_map, disp_map, acc_map, weights, depth_map = star_network(pts, viewdirs, z_vals, rays_d, 
                                                                  is_coarse=False)
    
    # For visualization
    z_std = torch.std(z_samples, dim=-1, unbiased=False)  # [N_rays]

    return rgb_map0, depth_map0, acc_map0, rgb_map, depth_map, acc_map, z_std
    
def render_star_online(star_network, pts, viewdirs, z_vals, rays_o, rays_d, N_importance, pose, step):
    ...
    rgb_map0, disp_map0, acc_map0, weights, depth_map0, entropy0, rgb_map_static0, rgb_map_dynamic0, \
        depth0_static, depth0_dynamic, transformed_pts0 = \
            star_network(pts, viewdirs, z_vals, rays_d, pose, is_coarse=True, step=step)    
    ...
    z_vals, _ = torch.sort(torch.cat([z_vals, z_samples], -1), -1)
    pts = rays_o[...,None,:] + rays_d[...,None,:] * z_vals[...,:,None] # [N_rays, N_samples + N_importance, 3]
    rgb_map, disp_map, acc_map, weights, depth_map, entropy, rgb_map_static, rgb_map_dynamic, depth_static, \
        depth_dynamic, transformed_pts = \
            star_network(pts, viewdirs, z_vals, rays_d, pose, is_coarse=False, step=step)

    extras = {}
    extras['rgb_dynamic'] = rgb_map_dynamic
    extras['rgb_static'] = rgb_map_static
    extras['transformed_pts'] = transformed_pts
    extras['rgb0'] = rgb_map0
    extras['depth0'] = depth_map0
    extras['acc0'] = acc_map0
    extras['z_std'] = torch.std(z_samples, dim=-1, unbiased=False)  # [N_rays]
    extras['rgb0_static'] = rgb_map_static0
    extras['rgb0_dynamic'] = rgb_map_dynamic0
    extras['entropy0'] = entropy0
    extras['transformed_pts0'] = transformed_pts0
    extras['depth_static'] = depth_static
    extras['depth_dynamic'] = depth_dynamic

    return rgb_map, depth_map, acc_map, extras, entropy
...
def raw2outputs_star(raw_alpha_static, raw_rgb_static, raw_alpha_dynamic, raw_rgb_dynamic, z_vals, rays_d, 
    ...
    T_s = torch.cumprod(torch.cat([torch.ones((alpha_static.shape[0], 1), device=device), 
                                   1.-alpha_static + 1e-10], -1), -1)[:, :-1]
    T_d = torch.cumprod(torch.cat([torch.ones((alpha_dynamic.shape[0], 1), device=device), 
                                   1.-alpha_dynamic + 1e-10], -1), -1)[:, :-1]
    #T = T_s * T_d 
    T = torch.cumprod(torch.cat([torch.ones((alpha_total.shape[0], 1), device=device), 
                                 1.-alpha_total + 1e-10], -1), -1)[:, :-1]
    rgb_map = torch.sum(T[...,None] * (alpha_static[...,None] * rgb_static + \
                                       alpha_dynamic[...,None] * rgb_dynamic), dim=-2)
    ...
    if ret_entropy:
        ...
        return rgb_map, disp_map, acc_map, weights, depth_map, entropy, rgb_map_static, rgb_map_dynamic, \
            depth_static, depth_dynamic

    return rgb_map, disp_map, acc_map, weights, depth_map, rgb_map_static, rgb_map_dynamic, depth_static, \
        depth_dynamic
...
def compute_entropy(alpha_static, alpha_dynamic=None):
    ...
    entropy = -torch.mean(alpha_static * torch.log(alpha_static_clamp) + \
                          (1 - alpha_static) * torch.log1p(-alpha_static_clamp))
    entropy += -torch.mean(alpha_dynamic * torch.log(alpha_dynamic_clamp) + \
