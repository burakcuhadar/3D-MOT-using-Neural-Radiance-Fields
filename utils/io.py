...
def load_ckpt_online(ckpt_path, star_model, optimizer, pose_optimizer, scheduler):
    ckpt = torch.load(ckpt_path)
...
def save_ckpt_star_online(path, star_model, optimizer, pose_optimizer, scheduler, step, k):
    torch.save({
        'step': step,
        'star_model': star_model.state_dict(),
        'optimizer': optimizer.state_dict(),
